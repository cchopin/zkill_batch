...
def update_index():
    ...
    # Exclude index.html and ranking files if present
    report_files = [f for f in report_files if os.path.basename(f) not in ["index.html", "ishtar_ranking.html", "mtu_ranking.html"]]
    report_files.sort(reverse=True)  # Sort in reverse order for most recent first

    # Generate list of links for monthly reports
    links = ""
...
<head>
    ...
    <style>
        ...
        .mtu-link {{
            background: linear-gradient(135deg, #2a2a2a, #004);
            border: 2px solid var(--accent) !important;
            grid-column: span 2;
        }}
        .mtu-link:hover {{
            box-shadow: 0 12px 30px rgba(0,180,255,0.8);
        }}
        .mtu-link a {{
